...
{
	if (!theRocketFile.openFile(ios::in) || !theAlienShipFile.openFile(ios::in) || theBulletsFile.openFile(ios::in)) //open file for input output
	{
...
void cSceneMgr::renderAlienSpaceShips(vector<cAlienShip*> theAlienShips)
{
	...
	{
		theAlienShips[draw]->render(theRenderer, &theAlienShips[draw]->getSpriteDimensions(), &theAlienShips[draw]->getSpritePos(),
			theAlienShips[draw]->getSpriteRotAngle(), &theAlienShips[draw]->getSpriteCentre(), theAlienShips[draw]->getSpriteScale());
	}
}
...
void cSceneMgr::renderPlaying()
{
	...
	//renderElementsObject.updateRenderedLives("LivesValue", gameTextList, updateLives, livesValue);
	// render lives
	if (updateLives)
	{
	...
	for (vector<cAlienShip*>::iterator alienShipIterator = theAlienShips.begin(); alienShipIterator != theAlienShips.end(); ++alienShipIterator)
	{
		...
		if ((*alienShipIterator)->getRateOfFire() <= 0)
		{
			...
			theBullets[numBullets]->setSpritePos({ (*alienShipIterator)->getSpritePos().x + (*alienShipIterator)->getSpriteCentre().x - theBullets[numBullets]->getSpriteCentre().x
				,(*alienShipIterator)->getSpritePos().y + (*alienShipIterator)->getSpriteCentre().x * 2 + 30 });	//question 
...
void cSceneMgr::updateScenes(float deltaTime)
{
	...
	case PLAYING:
	{
		...
		if (autoSaveTimer >= 2)
		{
			...
			{
				cout << "Could not open specified files '" << theRocketFile.getFileName() << "'. Error " << SDL_GetError() << endl;
			}
			else
			{
				cout << "File '" << theRocketFile.getFileName() << "' opened for input!" << endl;
...
void cSceneMgr::clearFiles()
{
	if (!clearedFiles)
	{
		if (!theRocketFile.openFile(ios::out) || !theAlienShipFile.openFile(ios::out) || theBulletsFile.openFile(ios::out)) //open file for input output
		{
...
void cSceneMgr::setBullets(vector<cBullet*> theBullets)
{
	this->theBullets = theBullets;
}

void cSceneMgr::setAlienShips(vector<cAlienShip*> theAlienShips)
{
	this->theAlienShips = theAlienShips;
}
...
void cSceneMgr::setUpdateLives(bool updateLives)
{
	this->updateLives = updateLives;
}

void cSceneMgr::setUpdateScore(bool updateScore)
{
...
int cSceneMgr::getLivesValue()
{
	return livesValue;
}

int cSceneMgr::getScoreValue()
{
	return scoreValue;
}

bool cSceneMgr::getUpdateLives()
{
	return updateLives;
}

bool cSceneMgr::getUpdateScore()
{
	return updateScore;
}

vector<cAlienShip*> cSceneMgr::getAlienShips()
{
	return theAlienShips;
}

vector<cBullet*> cSceneMgr::getBullets()
{
	return theBullets;
}

cRocket cSceneMgr::getRocket()
{
	return theRocket;
}

SDL_Point cSceneMgr::getAreaClicked()
{
	return theAreaClicked;
}

gameState cSceneMgr::getGameState()
{
	return theGameState;
}